...
; stay blocked forever, and report the failure.
        ...
        mov     eax, [edi+XhciCmdRingReg]
        or      eax, 4                  ; Command Abort
...
; 9. The event ring: one segment described by a single ERST entry.
        ...
        call    xhci_write64
; 10. Hook the interrupt. An interrupt line of 0 or 255 means the firmware has
...
; about. Set the PCI Interrupt Disable bit as well.
        ...
        invoke  PciWrite16, dword [.bus], dword [.devfn], 4, eax
.irq_done:
; 11. Configure the primary interrupter; leave interrupts disabled when the
...
; 12. Build the map of the ports to serve.
        mov     edi, [esi+xhci_controller.MMIOBase-XCD]
